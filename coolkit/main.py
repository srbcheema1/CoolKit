...
def safe_main():
    ...
    if(first_arg == "init"):
        ...
        if(second_arg == "user"):
            ...
            if(not c_name):
                if(Args.check_init()):
                    print(Colour.YELLOW+'contest not set, use `coolkit set -c <contest num>`'+Colour.END)
            ...
            if(not p_name):
                print(Colour.YELLOW+'problem not set, use `coolkit set -p <problem name>`'+Colour.END)
